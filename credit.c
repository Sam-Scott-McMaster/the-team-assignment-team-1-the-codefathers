...
void spend_money_credit(double *credit_balance, double amount, const char *folder_name, const char *username, const char *date) {
    ...
    if (*credit_balance <= 1000.00) {
        *credit_balance += amount;
        ...
        add_transaction_to_user_file(folder_name, username, "Credit", amount, date, *credit_balance, "Credit Limit Reached.");
        printf("Transaction Date: %s\n", date);
...
void pay_off_credit(double *debit_balance, double *credit_balance, double amount, const char *folder_name, const char *username, const char *date) {
    if (amount <= *debit_balance) {
        ...
        add_transaction_to_user_file(folder_name, username, "Credit", amount, date, *credit_balance, "Paid off Credit Balance.");
        printf("Transaction Date: %s\n", date);
